...
object CatsInstances extends CatsInstances01 {
  def semi2comp[->[_,_]](implicit lp: LowPriority, s: Semicategory[->]): Compose[->] = new ComposeFromSemicat[->] { val S = s }
  def comp2semi[->[_,_]](implicit c: Compose[->]): Semicategory[->] = new SemicatFromCompose[->] { val C = c }
...
trait CatsInstances01 {
  def sub2cat[->[_,_]](implicit lp: LowPriority, s: Subcat.Aux[->, Trivial.T1]): Category[->] = new CategoryFromSubcat[->] { val S = s }
  def cat2sub[->[_,_]](implicit c: Category[->]): Subcat.Aux[->, Trivial.T1] = new SubcatFromCategory[->] { val C = c }
