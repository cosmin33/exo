...
object KleisModule {
  ...
  def kleisCartesian[->[_,_], ⨂[_,_], F[_], T[_], Term](implicit
    ...
    new Cartesian[λ[(a,b) => a -> F[b]], ⨂] {
      type TC[a] = cc.TC[a]
      type Id = Term
      private implicit val sdf: TC[Term] = t.terminalTC
      def C: Subcat.Aux[λ[(a, b) => a -> F[b]], cc.TC] = KleisModule.category
      def bifunctor: Endobifunctor[λ[(a, b) => a -> F[b]], ⨂] = KleisModule.bifunctor
      def associate  [X: TC, Y: TC, Z: TC]: X ⨂ Y ⨂ Z -> F[X ⨂ (Y ⨂ Z)] = cc.C.andThen(cc.associate[X, Y, Z], mon.pure)
      def diassociate[X: TC, Y: TC, Z: TC]: X ⨂ (Y ⨂ Z) -> F[X ⨂ Y ⨂ Z] = cc.C.andThen(cc.diassociate[X, Y, Z], mon.pure)
