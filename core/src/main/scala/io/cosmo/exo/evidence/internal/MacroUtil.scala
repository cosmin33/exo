...
import io.cosmo.exo.evidence.TypeId
...
sealed abstract class Shared[C <: blackbox.Context] {
  val c: C
  import c.universe._
  import definitions.{AnyClass, NothingClass}
...
final class MacroUtil(val c: blackbox.Context) extends Shared[blackbox.Context] {
  ...
  def mkInhabited[A](implicit A: c.WeakTypeTag[A]): c.Tree =
    weakTypeOf[A].dealias match {
      case tpe@SingleType(_, path) =>
        q"""_root_.io.cosmo.exo.evidence.inhabitance.Inhabited.value[$tpe]($path.asInstanceOf[$tpe])"""
      case tpe@ConstantType(value) =>
        q"""_root_.io.cosmo.exo.evidence.inhabitance.Inhabited.value[$tpe]($value)"""
      case tpe@ThisType(_) =>
        q"""_root_.io.cosmo.exo.evidence.inhabitance.Inhabited.value[$tpe](this)"""

      case tpe =>
        inhabitants.find { case (t, _) => t <:< tpe } match {
          case Some((_, tree)) => q"""_root_.io.cosmo.exo.evidence.inhabitance.Inhabited.value[$tpe]($tree)"""
  ...
  def mkUninhabited[A](implicit A: c.WeakTypeTag[A]): c.Tree =
    weakTypeOf[A].dealias match {
      case tpe if tpe <:< NothingType =>
        q"""_root_.io.cosmo.exo.evidence.inhabitance.Uninhabited.witness[$tpe](a => a)"""
  ...
  def mkInjective[F[_]](implicit F: c.WeakTypeTag[F[_]]): c.Tree =
    if (isInjective[F](F))
      q"_root_.io.cosmo.exo.evidence.variance.Injective.force[${F.tpe}](_root_.io.cosmo.exo.evidence.internal.Unsafe.unsafe)"
    else
      c.abort(c.enclosingPosition, s"Can't prove that ${F.tpe} is injective.")

  def mkConstant[F[_]](implicit F: c.WeakTypeTag[F[_]]): c.Tree =
    if (isConstant[F](F))
      q"_root_.io.cosmo.exo.evidence.variance.Constant.force[${F.tpe}](_root_.io.cosmo.exo.evidence.internal.Unsafe.unsafe)"
    else
      c.abort(c.enclosingPosition, s"Can't prove that ${F.tpe} is injective.")
  ...
  def mkApart[A : c.WeakTypeTag, B : c.WeakTypeTag]: c.Tree = {
    ...
    if (isConcreteType(ta) && isConcreteType(tb) && !(ta <:< tb && tb <:< ta)) {
      ...
      q"_root_.io.cosmo.exo.evidence.internal.Unsafe.apart[$ta, $tb]($ca, $cb)"
    } else {
  ...
  def mkWeakApart[A : c.WeakTypeTag, B : c.WeakTypeTag]: c.Tree = {
    ...
    if (isConcreteType(ta) && isConcreteType(tb) && !(ta <:< tb && tb <:< ta)) {
      // val ca = makeConcreteType(ta)
      // val cb = makeConcreteType(tb)
      q"""_root_.io.cosmo.exo.evidence.internal.Unsafe.weakApart[$ta, $tb]"""
    } else {
      c.abort(c.enclosingPosition, s"Could not prove that $ta =!= $tb.")
    }
  }
