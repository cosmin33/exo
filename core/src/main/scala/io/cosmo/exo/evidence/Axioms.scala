...
object Axioms {
  ...
  def isConsistency[A, B](ab: ((A === B) => Void) => Void): A === B = Unsafe.is[A, B]

  def asConsistency[A, B](ab: ((A <~< B) => Void) => Void): A <~< B = Unsafe.as[A, B]

  def leibnizConsistency[L, H >: L, A >: L <: H, B >: L <: H](ab: (Leibniz[L, H, A, B] => Void) => Void)
  : Leibniz[L, H, A, B] = Leibniz.refl[A].asInstanceOf[Leibniz[L, H, A, B]]

  def liskovConsistency[L, H >: L, A >: L <: H, B >: L <: H](ab: (Liskov[L, H, A, B] => Void) => Void)
  : Liskov[L, H, A, B] = Liskov.refl[A].asInstanceOf[Liskov[L, H, A, B]]
  ...
  final class TCExtensionality[F[_], G[_]](val b: Boolean = true) extends AnyVal {
    ...
    def applyT(f: TypeHolder[T] => (F[T] === G[T])): F =~= G = apply(f(TypeHolder[T]))
  }
