...
object Sigma {
  ...
  final def mapK[A, F[_], G[_]](s: Sigma[A, F])(fg: F ~> G): Sigma[A, G] = Sigma(s.first)(fg.apply(s.second))
...
//trait SigmaK[+F[_], +A[_[_]]] {
//  type First[x] <: F[x]
//  def second: A[First]
//}
//object SigmaK {
//  def apply[F[_], A[_[_]]](a: A[F]): SigmaK[F, A] = new SigmaK[F, A] {
//    type First[x] = F[x]
//    val second: A[First] = a
//  }
//}
