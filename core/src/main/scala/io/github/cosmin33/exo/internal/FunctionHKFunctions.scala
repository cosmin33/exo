package io.github.cosmin33.exo.internal

import io.github.cosmin33.exo._
import io.github.cosmin33.exo.categories._
import io.github.cosmin33.exo.functors._
import io.github.cosmin33.exo.syntax._
import io.github.cosmin33.exo.internal.any._
...
trait FunctionHKFunctions {
  ...
  def initiate[A[_[_]]]: VoidH ≈> A = ∀~.mk[VoidH ≈> A].from(identity)
  def terminate[A[_[_]]]: A ≈> UnitH = ∀~.mk[A ≈> UnitH].from(_ => ())
  ...
  object product {
    ...
    def idl[A[_[_]]]: ([f[_]] =>> (UnitH[f], A[f])) ≈> A = ∀~.mk[([f[_]] =>> (UnitH[f], A[f])) ≈> A].from(_._2)
    def idr[A[_[_]]]: ([f[_]] =>> (A[f], UnitH[f])) ≈> A = ∀~.mk[([f[_]] =>> (A[f], UnitH[f])) ≈> A].from(_._1)
    def coidl[A[_[_]]]: A ≈> ([f[_]] =>> (UnitH[f], A[f])) = ∀~.mk[A ≈> ([f[_]] =>> (UnitH[f], A[f]))].from(fa => ((), fa))
    def coidr[A[_[_]]]: A ≈> ([f[_]] =>> (A[f], UnitH[f])) = ∀~.mk[A ≈> ([f[_]] =>> (A[f], UnitH[f]))].from(fa => (fa, ()))
  ...
  object coproduct {
    ...
    def coidl[A[_[_]]]: ([f[_]] =>> Either[VoidH[f], A[f]]) ≈> A = ∀~.mk[([f[_]] =>> Either[VoidH[f], A[f]]) ≈> A].from(_.fold(identity, identity))
    def coidr[A[_[_]]]: ([f[_]] =>> Either[A[f], VoidH[f]]) ≈> A = ∀~.mk[([f[_]] =>> Either[A[f], VoidH[f]]) ≈> A].from(_.fold(identity, identity))
    def idl[A[_[_]]]: A ≈> ([f[_]] =>> Either[VoidH[f], A[f]]) = ∀~.mk[A ≈> ([f[_]] =>> Either[VoidH[f], A[f]])].from(_.asRight)
    def idr[A[_[_]]]: A ≈> ([f[_]] =>> Either[A[f], VoidH[f]]) = ∀~.mk[A ≈> ([f[_]] =>> Either[A[f], VoidH[f]])].from(_.asLeft)
