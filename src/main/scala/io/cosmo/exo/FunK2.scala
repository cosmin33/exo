...
object FunK2Helpers:
  trait Funk2BifTuple extends Endobifunctor[FunK2, Tuple2]:
    def bimap[A, X, B, Y](left: FunK2[A, X], right: FunK2[B, Y]): FunK2[(A, B), (X, Y)] =
      FunK2(~~>.product.bimap(left.fn, right.fn))(using
        IsKind2.injTuple(using left.kindA, right.kindA),
        IsKind2.injTuple(using left.kindB, right.kindB)
      )
  ...
  trait Funk2BifEither extends Endobifunctor[FunK2, Either]:
    def bimap[A, X, B, Y](l: FunK2[A, X], r: FunK2[B, Y]): FunK2[Either[A, B], Either[X, Y]] =
      FunK2(~~>.coproduct.bimap(l.fn, r.fn))(using IsKind2.injEither(using l.kindA, r.kindA), IsKind2.injEither(using l.kindB, r.kindB))
  ...
  trait Funk2Subcat extends Distributive.Proto[FunK2, IsKind2, Tuple2, TypeK2[UnitK2], Either, TypeK2[VoidK2]]:
    ...
    def distribute[A, B, C](using ia: IsKind2[A], ib: IsKind2[B], ic: IsKind2[C]): FunK2[(A, Either[B, C]), Either[(A, B), (A, C)]] =
      FunK2(~~>.distribute[ia.Type, ib.Type, ic.Type])(using IsKind2.injTuple[A, Either[B, C]], IsKind2.injEither[(A, B), (A, C)])
  ...
  trait Funk2CccTuple extends Ccc.Proto[FunK2, Tuple2, IsKind2, TypeK2[UnitK2], FunK2]:
    ...
    def fst[A, B](using ia: IsKind2[A], ib: IsKind2[B]): FunK2[(A, B), A] =
      FunK2(~~>.product.fst[ia.Type, ib.Type])(using IsKind2.injTuple[A, B], ia)
    def snd[A, B](using ia: IsKind2[A], ib: IsKind2[B]): FunK2[(A, B), B] =
      FunK2(~~>.product.snd[ia.Type, ib.Type])(using IsKind2.injTuple[A, B], ib)
    def diag[A](using ia: IsKind2[A]): FunK2[A, (A, A)] =
      FunK2(~~>.product.diag[ia.Type])(using ia, IsKind2.injTuple(using ia, ia))
    def &&&[X, Y, Z](f: FunK2[X, Y], g: FunK2[X, Z]): FunK2[X, (Y, Z)] =
      ...
      )(using f.kindA, IsKind2.injTuple(using f.kindB, g.kindB))
    def idl  [A](using ia: IsKind2[A]): FunK2[(TypeK2[UnitK2], A), A] =
      FunK2(~~>.product.idl[ia.Type])(using IsKind2.injTuple[TypeK2[UnitK2], A], ia)
    def coidl[A](using ia: IsKind2[A]): FunK2[A, (TypeK2[UnitK2], A)] =
      FunK2(~~>.product.coidl[ia.Type])(using ia, IsKind2.injTuple[TypeK2[UnitK2], A])
    def idr  [A](using ia: IsKind2[A]): FunK2[(A, TypeK2[UnitK2]), A] =
      FunK2(~~>.product.idr[ia.Type])(using IsKind2.injTuple[A, TypeK2[UnitK2]], ia)
    def coidr[A](using ia: IsKind2[A]): FunK2[A, (A, TypeK2[UnitK2])] =
      FunK2(~~>.product.coidr[ia.Type])(using ia, IsKind2.injTuple[A, TypeK2[UnitK2]])
    def braid[A, B](using ia: IsKind2[A], ib: IsKind2[B]): FunK2[(A, B), (B, A)] =
      FunK2(~~>.product.braid[ia.Type, ib.Type])(using IsKind2.injTuple[A, B], IsKind2.injTuple[B, A])
    def curry[A, B, C](f: FunK2[(A, B), C]): FunK2[A, FunK2[B, C]] = {
      val c: IsKind2.Aux[C, f.TypeB] = f.kindB
      val (ia, ib) = f.kindA.pairInjectivity[Tuple2, A, B]
      val fun = IsKind2.injectivity(f.kindA, IsKind2.injTuple[A, B](using ia, ib)).subst[[f[_,_]] =>> f ~~> f.TypeB](f.fn)
      FunK2[ia.Type, [o,p] =>> ib.Type[o,p] => c.Type[o,p], A, FunK2[B, C]](~~>.product.curry(fun))(using ia, IsKind2.injFunction[B, C](using ib, c))
    }
    def uncurry[A, B, C](f: FunK2[A, FunK2[B, C]]): FunK2[(A, B), C] = {
      val a: IsKind2.Aux[A, f.TypeA] = f.kindA
      val (ib, ic) = f.kindB.pairInjectivity[FunK2, B, C]
      val fun = IsKind2.injectivity(f.kindB, IsKind2.injFunction[B, C](using ib, ic)).subst[[f[_,_]] =>> f.TypeA ~~> f](f.fn)
      FunK2[[o,p] =>> (a.Type[o,p], ib.Type[o,p]), ic.Type, (A, B), C](~~>.product.uncurry(fun))(using IsKind2.injTuple[A, B](using a, ib), ic)
    }
  ...
  trait Funk2CocartesianEither extends Cartesian.Proto[Opp[FunK2], Either, IsKind2, TypeK2[VoidK2]]:
    ...
    def associate  [X, Y, Z](using ix: IsKind2[X], iy: IsKind2[Y], iz: IsKind2[Z]): FunK2[Either[X, Either[Y, Z]], Either[Either[X, Y], Z]] =
      FunK2(~~>.coproduct.associate[ix.Type, iy.Type, iz.Type])(using
        IsKind2.injEither[X, Either[Y, Z]],
        IsKind2.injEither[Either[X, Y], Z]
      )
    ...
    def fst[A, B](using ia: IsKind2[A], ib: IsKind2[B]): FunK2[A, Either[A, B]] =
      FunK2(~~>.coproduct.inl[ia.Type, ib.Type])(using ia, IsKind2.injEither[A, B])
    def snd[A, B](using ia: IsKind2[A], ib: IsKind2[B]): FunK2[B, Either[A, B]] =
      FunK2(~~>.coproduct.inr[ia.Type, ib.Type])(using ib, IsKind2.injEither[A, B])
    def diag[A](using ia: IsKind2[A]): FunK2[Either[A, A], A] =
      FunK2(~~>.coproduct.codiag[ia.Type])(using IsKind2.injEither[A, A], ia)
    def &&&[X, Y, Z](f: FunK2[Y, X], g: FunK2[Z, X]): FunK2[Either[Y, Z], X] =
      ...
      )(using IsKind2.injEither(using f.kindA, g.kindA), f.kindB)
    def idl  [A](using ia: IsKind2[A]): FunK2[A, Either[TypeK2[VoidK2], A]] =
      FunK2(~~>.coproduct.idl[ia.Type])(using ia, IsKind2.injEither[TypeK2[VoidK2], A])
    def coidl[A](using ia: IsKind2[A]): FunK2[Either[TypeK2[VoidK2], A], A] =
      FunK2(~~>.coproduct.coidl[ia.Type])(using IsKind2.injEither[TypeK2[VoidK2], A], ia)
    def idr  [A](using ia: IsKind2[A]): FunK2[A, Either[A, TypeK2[VoidK2]]] =
      FunK2(~~>.coproduct.idr[ia.Type])(using ia, IsKind2.injEither[A, TypeK2[VoidK2]])
    def coidr[A](using ia: IsKind2[A]): FunK2[Either[A, TypeK2[VoidK2]], A] =
      FunK2(~~>.coproduct.coidr[ia.Type])(using IsKind2.injEither[A, TypeK2[VoidK2]], ia)
    def braid[A, B](using ia: IsKind2[A], ib: IsKind2[B]): FunK2[Either[B, A], Either[A, B]] =
      FunK2(~~>.coproduct.braid[ib.Type, ia.Type])(using IsKind2.injEither[B, A], IsKind2.injEither[A, B])
